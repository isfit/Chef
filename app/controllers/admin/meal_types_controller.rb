class Admin::MealTypesController < ApplicationController
  ...
  def create
    ...
    respond_to do |format|
      if @meal_type.save
        format.html { redirect_to admin_meal_types_path, notice: 'Maaltidstype ble opprettet.' }
  ...
  def update
    ...
    respond_to do |format|
      if @meal_type.update_attributes(params[:meal_type])
        format.html { redirect_to admin_meal_types_path, notice: 'Maaltidstype ble oppdatert.' }
